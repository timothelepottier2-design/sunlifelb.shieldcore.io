...
import radioSrcTheme1 from "../../assets/images/_radio.png";
import radioSrcTheme2 from "../../assets/images/_radio2.png";
import radioSrcTheme3 from "../../assets/images/_radio3.png";
import radioSrcTheme4 from "../../assets/images/_radio4.png";
...
const VerticalRadio: React.FC = ({ inputValue, setInputValue, visible, gameTime, radioVolume, alerts, playersInRadio, jammerHud, visibleSoundBar, visibleMenu, tab, onRadio, theme, channels, handleCloseAlert, setTab, leaveChannel, handleRadioPower, handleVolumeSet, handleKeyDown, handleConnect, handleMenu, handleMutePlayer, handleMemberMenu }) => {
    const inputRef = useRef<HTMLInputElement>(null);
    const nodeRef = useRef(null);
    const [dragPosition, setDragPosition] = useState({ x: 0, y: 0 });

    useEffect(() => {
        const position = localStorage.getItem("verticalRadioPosition");
        if (position) {
    ...
    const handleResetPosition = () => {
        ...
        localStorage.removeItem("verticalRadioPosition");
    };

    const handleDragStop = (data: DraggableData) => {
        localStorage.setItem("verticalRadioPosition", JSON.stringify({ x: data.x, y: data.y }));
    };
            ...
            <Draggable nodeRef={nodeRef} bounds=".drag-container" scale={1} handle=".handle" position={dragPosition} onDrag={(e, data) => setDragPosition({ x: data.x, y: data.y })} onStop={(e, data) => handleDragStop(data)} disabled={!visible}>
                <Transition unmount={false} show={visible} as={Fragment} enter="transition-all duration-300 ease-out" enterFrom="bottom-[-70%] opacity-0" enterTo="bottom-[5vh] opacity-1" leave="transition-all duration-300 ease-in" leaveFrom="bottom-[5vh] opacity-1" leaveTo="bottom-[-70%] opacity-0">
                    <div ref={nodeRef} className="radio absolute bottom-[5vh] right-[5vh] mx-auto px-3 z-[100]">
                        {theme == "theme1" && <img className="w-[26vh]" src={radioSrcTheme1} alt="Radio" />}
                        {theme == "theme2" && <img className="w-[26vh]" src={radioSrcTheme2} alt="Radio" />}
                        {theme == "theme3" && <img className="w-[26vh]" src={radioSrcTheme3} alt="Radio" />}
                        {theme == "theme4" && <img className="w-[26vh]" src={radioSrcTheme4} alt="Radio" />}
                        {theme == "theme5" && <img className="w-[26vh]" src={radioSrcTheme4} alt="Radio" />}

                        <div className="bottom-[5.8vh] right-[3.3vh] w-[20.5vh] h-[36.2vh] absolute flex flex-col justify-between">
                            {jammerHud && onRadio && <JammerHud />}

                            {theme == "theme1" && <img className="absolute top-0 left-0 w-full h-full z-0" src={bg1} alt="Background" />}
                            {theme == "theme2" && <img className="absolute top-0 left-0 w-full h-full z-0" src={bg2} alt="Background" />}
                            {theme == "theme3" && <img className="absolute top-0 left-0 w-full h-full z-0" src={bg3} alt="Background" />}
                            {theme == "theme4" && <img className="absolute top-0 left-0 w-full h-full z-0" src={bg4} alt="Background" />}
                            {theme == "theme5" && <img className="absolute top-0 left-0 w-full h-full z-0" src={bg5} alt="Background" />}

                            {!isEnvBrowser() && <BootAnimation />}
                            <div className="w-full h-full z-10">
                                <div className="p-[4px] px-[8px] flex items-center justify-between bg-black">
                                    {/* font-family: Inter; font-size: 7.88px; font-weight: 800; line-height: 9.54px; text-align: left; */}
                                    <div className="text-[8px] text-white/25">{gameTime}</div>

                                    <div className="flex items-center justify-between">
                                        <div className="mr-3">
                                            <svg width="9" height="9" viewBox="0 0 9 9" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <rect width="2.28985" height="7.5238" rx="0.327122" transform="matrix(-1 0 0 1 8.86523 0.600586)" fill="white" fillOpacity="0.24" />
                                                <rect width="2.28985" height="5.56107" rx="0.327122" transform="matrix(-1 0 0 1 5.5957 2.56348)" fill="white" fillOpacity="0.24" />
                                                <rect width="2.28985" height="3.27122" rx="0.327122" transform="matrix(-1 0 0 1 2.32227 4.85303)" fill="white" fillOpacity="0.24" />
                                            </svg>
                                        </div>

                                        <div className="flex items-center justify-between">
                                            <span className="text-[8px] font-extrabold text-white/25 mr-[2px]">
                                                <small>%</small>45
                                            </span>
                                            <svg width="15" height="9" viewBox="0 0 15 9" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <path
                                                    d="M12.7875 7.25188V6.14333H13.2032C13.6509 6.14333 14.0346 5.75949 14.0346 5.31192V3.37196C14.0346 2.92438 13.6509 2.54055 13.2032 2.54055H12.7875V1.432C12.7875 0.984559 12.4038 0.600586 11.9561 0.600586H0.870577C0.422862 0.600586 0.177734 1.01546 0.177734 1.46304V7.22084C0.177734 7.66855 0.422862 8.08329 0.870577 8.08329H11.9561C12.4038 8.08329 12.7875 7.69959 12.7875 7.25188ZM1.28628 7.25188C1.24568 7.25188 1.14771 7.15405 1.14771 7.11331V1.70913C1.14771 1.66853 1.24568 1.57057 1.28628 1.57057H11.6789C11.7197 1.57057 11.8175 1.66853 11.8175 1.70913V7.11331C11.8175 7.15405 11.7197 7.25188 11.6789 7.25188H1.28628ZM1.42485 1.8477V6.97474H4.33479V1.88137L1.42485 1.8477Z"
                                                    fill="white"
                                                    fillOpacity="0.24"
                                                />
                                            </svg>
                                ...
                                {!visibleMenu && (
                                    <div className="w-full flex items-center justify-center flex-col mt-[34px] smdesktop:mt-[15px]">
                                        <span className="text-[12px] text-white/30 font-medium smdesktop:text-[8px]">23 July 2024</span>
                                        <span className="time-font text-white/30 text-[32px] font-normal leading-tight smdesktop:text-[22px]">{gameTime}</span>
                                    </div>
                                )}

                                <div className="flex w-full h-full absolute top-0 left-0 px-[14px] pt-[18px] pb-[24px] smdesktop:px-[5px] smdesktop:pb-[20px]">
                                    <div className="flex-1 overflow-scroll no-scrollbar">
                                        <div className="min-h-full flex flex-col justify-end">
                                            <Transition show={visibleMenu} as={Fragment} enter="transition-all duration-300 ease-out" enterFrom="opacity-0 transform -translate-y-20" enterTo="opacity-100 transform translate-y-0" leave="transition-all duration-300 ease-in" leaveFrom="opacity-100 transform translate-y-0" leaveTo="opacity-0 transform -translate-y-20">
                                                <div className="shrink-0 flex items-center justify-between mb-[8px] mt-[20px] overflow-scroll no-scrollbar smdesktop:mt-[8px]">
                                            ...
                                            {!visibleMenu && (
                                                <div className="relative w-full h-[55px] smdesktop:h-[45px]">
                                                    {alerts.map((alert: any, index: number) => (
                                                        <Alert key={index} alert={alert} index={index} onClose={handleCloseAlert} isLast={index === alerts.length - 1} />
                                                    ))}
                                                </div>
                                            )}

                                            <Transition unmount={false} show={tab === "members" || !visibleMenu} as={Fragment} enter="transition-all duration-300 ease-out" enterFrom="opacity-0 transform translate-y-20" enterTo="opacity-100 transform translate-y-0" leave="transition-all duration-300 ease-in" leaveFrom="opacity-100 transform translate-y-0" leaveTo="opacity-0 transform translate-y-20">
                                                <div className="relative custom-radial-gradient rounded-[4px] shrink-0 overflow-hidden">
                                                    <img src={bgArrow} className="w-full absolute top-0 right-0 z-0" />
                                                    <div className="relative flex justfiy-between items-stretch z-10 smdesktop:flex-col">
                                                        <div className="w-3/4 py-[14px] pl-[14px] pr-[23px] smdesktop:w-full smdesktop:p-[10px] smdesktop:pb-1">
                                                            ...
                                                            <span className="block text-[1.8vh] text-white">CHANNEL</span>
                                                            <span className="flex items-center">
                                                                <span className="text-primary truncate text-[0.8vh] smdesktop:text-[6px]">{onRadio ? "Connected" : "Not Connected"}</span>
                                                                <svg className="shrink-0 ml-[5px] smdesktop:w-[8px] smdesktop:h-[7px]" width="12" height="11" viewBox="0 0 12 11" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                        ...
                                                        <div className="w-1/4 flex flex-col w-25 p-[6px] bg-black/10 shrink-0 gap-2 smdesktop:flex-row smdesktop:w-full smdesktop:gap-1">
                                                            ...
                                                            <button className="handle flex-1 flex items-center justify-center bg-white/10 px-[6px] py-[4px] rounded-[1px] hover:opacity-80 smdesktop:mb-0 smdesktop:p-[2px] cursor-grab">
                                            ...
                                            <Transition show={visibleMenu && tab === "members"} as={Fragment} enter="transition-all duration-300 ease-out" enterFrom="opacity-0 transform translate-y-20" enterTo="opacity-100 transform translate-y-0" leave="transition-all duration-300 ease-in" leaveFrom="opacity-100 transform translate-y-0" leaveTo="opacity-0 transform translate-y-20">
                                                <div className="relative grow custom-radial-gradient rounded-[4px] mt-[12px] overflow-hidden smdesktop:mt-[8px]">
                                                    ...
                                                    <div className="w-full h-full max-h-full p-[14px] smdesktop:p-[8px] flex flex-col">
                                                        <div className="flex items-center shrink-0 mb-[8px]">
                                                            ...
                                                            <span className="w-full bg-white/10 rounded h-[1px] mx-[5px] smdesktop:hidden"></span>
                                                            <span className="text-[10px] text-white/25 shrink-0 smdesktop:hidden">{onRadio && inputValue ? inputValue : "0"} MHZ</span>
                                                        </div>
                                                        <div className="overflow-auto no-scrollbar w-full grow flex flex-col gap-[6px]">
                                                            {playersInRadio &&
                                                                playersInRadio?.map((player: PlayerOnRadioType, index: number) => (
                                                                    <div key={index} className="w-full flex items-center px-[7px] py-[5px] bg-white/5 rounded-[4px]">
                                                                        ...
                                                                        <span className="text-[10px] smdesktop:text-[8px] truncate text-white mr-[4px]">{player.name}</span>

                                                                        <div className="flex items-center ml-auto">
                                                                            <button className={`mr-[10px] smdesktop:mr-[2px] ${player.isMuted ? "text-[#C65353]" : "text-white"}`} onClick={() => handleMutePlayer(player.source)}>
                                            ...
                                            <Transition show={visibleMenu && tab === "channels"} as={Fragment} enter="transition-all duration-300 ease-out" enterFrom="opacity-0 transform translate-y-20" enterTo="opacity-100 transform translate-y-0" leave="transition-all duration-300 ease-in" leaveFrom="opacity-100 transform translate-y-0" leaveTo="opacity-0 transform translate-y-20">
                                                <div>
                                                    <hr className="mb-[8px] border-white/5 smdesktop:my-[5px]" />

                                                    <div className="relative w-full custom-gradient-button rounded-[4px] mr-[14px]" onClick={() => setTab("channels")}>
                                                        <img src={bgArrowLeftButton} className="absolute top-0 left-0 h-full z-[-1]" />

                                                        <div className="flex items-center justify-between p-[12px] text-white text-[10px] smdesktop:p-[8px]">
                                                            <span className="flex items-center">
                                                    ...
                                                    <div className="w-full h-full max-h-full p-[14px] smdesktop:p-[8px] flex flex-col">
                                                        <div className="overflow-auto no-scrollbar w-full grow flex flex-col gap-[6px]">
                                                            {channels?.map((item: ChannelsType, index: number) => (
                                                                <div key={index} className="w-full flex items-center justify-between p-[7px] bg-white/10 rounded-[4px]">
                                                                    <div className="text-[12px] text-white smdesktop:text-[8px]">
                                                                        <span className="text-[12px] smdesktop:text-[8px] text-[#637CCA] mr-[2px]">#</span>
                                                                        {item.channel}
                                                                        <small>mhz</small>
                                                                    </div>
                            ...
                            <button className="block absolute w-full bottom-0 flex items-center justify-center h-[24px] z-10 hover:opacity-80 outline-none" onClick={handleMenu}>
                                <div className="rounded-[14px] w-[47px] h-[3px] bg-white"></div>
                            </button>
                        </div>

                        <div className="absolute overflow-hidden top-[28%] left-0 w-full h-[2vh] flex items-center justify-center">
                            <button className="w-[7vh] h-full rounded-full hover:bg-sky-200/10 outline-none" onClick={handleRadioPower}></button>
                        </div>

                        <div className="absolute overflow-hidden top-[43%] left-0 w-[1.5vh] h-[13vh] flex flex-col">
                            <button className="flex-1 hover:bg-sky-200/10 outline-none rounded-full" onClick={() => handleVolumeSet("volumeUp")}></button>
                            <button className="flex-1 hover:bg-sky-200/10 outline-none rounded-full" onClick={() => handleVolumeSet("volumeDown")}></button>
                        </div>

                        <div className="absolute overflow-hidden bottom-[1.2vh] left-[3.5vh] w-[20vh] h-[4.5vh]">
                            <div>
                                <Button
                                    onClick={() => {
                                ...
                                >
                                    <img src={plus} alt="Minus" style={{ mixBlendMode: "soft-light" }} />
                                </Button>
                            </div>
                            <div>
                                <Button
                                ...
                                >
                                    <img src={minus} alt="Minus" style={{ mixBlendMode: "soft-light" }} />
                                </Button>
                            </div>
                        </div>
                    </div>
