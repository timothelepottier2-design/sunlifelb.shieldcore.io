...
AddEventHandler(
    'aquiverPoker:updatePokerPlayerChips',
    function(amount)
        if amount ~= nil then
        ...
        if GetPlayerServerId(PlayerId()) == mainSrc then
            ...
            Citizen.CreateThread(
                function()
                    ...
                    print(('^2[NETDIAG][OBJET]^7 %s cl_3poker.lua:796 CreateObjectNoOffset NETWORKED chip'):format(GetCurrentResourceName()))
                    print(('^2[NETDIAG][OBJET]^7 %s cl_3poker.lua CreateObjectNoOffset NETWORKED chip'):format(GetCurrentResourceName()))
            ...
            if not InformationPlaying and activePokerTable == nil and activeChairData == nil then
                if closeToPokers then
                    ...
                    for k, v in pairs(SharedPokers) do
                        ...
                        if dist < 3.0 then
                            local resolvedViaBones = false

                            for i = 1, #Config3Poker.Tables, 1 do
                                local tableObj = GetClosestObjectOfType(playerpos, 3.0, Config3Poker.GetTableHash(Config3Poker.Tables[i]), false)

                                if DoesEntityExist(tableObj) then
                                    letSleep = false

                                    for chairBone, chairId in pairs(Config3Poker.PokerChairs) do
                                        local boneIndex = GetEntityBoneIndexByName(tableObj, chairBone)
                                        -- index -1 = le bone n'existe pas sur ce modèle (ex: table de braquage décorative).
                                        if boneIndex and boneIndex ~= -1 then
                                            local chaircoords = GetWorldPositionOfEntityBone(tableObj, boneIndex)
                                            if chaircoords then
                                                resolvedViaBones = true
                                                if #(playerpos - chaircoords) < 1.5 and (not closeTable.ChairCoords or #(playerpos - chaircoords) < #(playerpos - closeTable.ChairCoords))then
                                                    local chairrotation = GetWorldRotationOfEntityBone(tableObj, boneIndex)
                                                    close = true
                            ...
                            -- Repli : le prop n'a pas de bones Chair_Base (ex: ch_prop_casino_poker_01b).
                            -- On utilise alors les sièges manuels définis dans la config (data.Chairs).
                            if not resolvedViaBones and v.data.Chairs then
                                letSleep = false

                                for _, chair in ipairs(v.data.Chairs) do
                                    local chaircoords = GetObjectOffsetFromCoords(v.data.Position, v.data.Heading, chair.x, chair.y, chair.z)
                                    if #(playerpos - chaircoords) < 2.0 and (not closeTable.ChairCoords or #(playerpos - chaircoords) < #(playerpos - closeTable.ChairCoords)) then
                                        close = true
                                        closeTable = {
                                            Poker = v,
                                            ChairId = chair.id,
                                            ChairCoords = chaircoords,
                                            ChairRotation = vector3(0.0, 0.0, v.data.Heading + (chair.h or 180.0))
                                        }
                                    end
                                end

                                if closeTable.ChairCoords and not playedHudSound then
                                    PlaySoundFrontend(-1, 'DLC_VW_RULES', 'dlc_vw_table_games_frontend_sounds', 1)
                                    playedHudSound = true
                                end
                            end
...
function drawfreameeMarker(position)
    DrawMarker(20, position, 0.0, 0.0, 0.0, 180.0, 0.0, 0.0, 0.3, 0.3, 0.3, 255, 117, 31, 225, true, true, 2, true, nil, nil, false)
end
...
RegisterCommand(
    ...
    function()
        ...
        for i = 1, #Config3Poker.Tables, 1 do
            local obj = GetClosestObjectOfType(playercoords, 3.0, Config3Poker.GetTableHash(Config3Poker.Tables[i]), false, false, false)
            if DoesEntityExist(obj) then
                found = true
                print(('[poker] table trouvée: entrée=%s model=%s entity=%s'):format(tostring(Config3Poker.Tables[i]), tostring(Config3Poker.GetTableHash(Config3Poker.Tables[i])), tostring(obj)))

                local objCoords = GetEntityCoords(obj)
                print(('[poker] origine prop: %.3f, %.3f, %.3f'):format(objCoords.x, objCoords.y, objCoords.z))

                local hasAnyBone = false
                for chairBone, chairId in pairs(Config3Poker.PokerChairs) do
                    local boneIndex = GetEntityBoneIndexByName(obj, chairBone)
                    if boneIndex and boneIndex ~= -1 then
                        hasAnyBone = true
                        local bc = GetWorldPositionOfEntityBone(obj, boneIndex)
                        print(('[poker]   %s (id %d): index=%d monde=%.3f, %.3f, %.3f dist=%.2f'):format(chairBone, chairId, boneIndex, bc.x, bc.y, bc.z, #(playercoords - bc)))
                    else
                        print(('[poker]   %s (id %d): ABSENT (index -1)'):format(chairBone, chairId))
                    end
                end

                if hasAnyBone then
                    print('[poker] => Ce modèle possède des bones Chair_Base : la détection native fonctionne.')
                else
                    print('[poker] => Aucun bone Chair_Base : repli sur les sièges manuels (data.Chairs).')
                end
            end
        end
