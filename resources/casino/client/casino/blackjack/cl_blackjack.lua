...
cfg.blackjackTables = {
    ...
    -- Tables du second casino (props déjà mappés, model -2126678982 = vw_prop_casino_blckjack_01b).
    -- dealerPos calculé : ~0.78m devant la table (sens du heading), z = plancher (~57.85),
    -- dealerHeading = tableHeading + 180.
    [4] = {
        dealerPos = vector3(7004.8494, 276.6570, 57.8546),
        dealerHeading = 45.0,
        tablePos = vector3(7004.2978515625, 277.20849609375, 56.84926986694336),
        tableHeading = 225.0,
        distance = 1000.0,
        prop = "vw_prop_casino_blckjack_01b"
    },
    [5] = {
        dealerPos = vector3(7002.0247, 273.8319, 57.8546),
        dealerHeading = 45.0,
        tablePos = vector3(7001.47314453125, 274.38348388671875, 56.84926986694336),
        tableHeading = 225.0,
        distance = 1000.0,
        prop = "vw_prop_casino_blckjack_01b"
    },
    [6] = {
        dealerPos = vector3(7000.8699, 271.7481, 57.8546),
        dealerHeading = 135.0,
        tablePos = vector3(7000.318359375, 271.1965637207031, 56.84926986694336),
        tableHeading = 315.0,
        distance = 1000.0,
        prop = "vw_prop_casino_blckjack_01b"
    },
...
Citizen.CreateThread(function()
    while true do 
        ...
        if drawCurrentHand then
            ...
            AddTextComponentString("Votre main est [~g~"..tostring(currentHand).."~s~], la main du croupier est [~g~"..tostring(dealersHand).."~s~]")
    ...
    while true do 
        if waitingForBetState then
            ...
            if IsControlPressed(0, 10) and GetGameTimer()-startTime > 250 then --Increase bet [pageup]
                ...
                if currentBetAmount + 100 <= maxBet then
                    currentBetAmount = currentBetAmount + 100
                else
                    notify("~r~Limite de mise atteinte (20,000 jetons)")
                end
            end    
...
function goToBlackjackSeat(blackjackSeatID)
    ...
    local sitEnterClip = blackjack_func_213(Local_198f_251)
    ...
    NetworkAddPedToSynchronisedScene(PlayerPedId(), Local_198f_255, "anim_casino_b@amb@casino@games@shared@player@", sitEnterClip, 2.0, -2.0, 13, 16, 2.0, 0) -- 8.0, -1.5, 157, 16, 1148846080, 0) ?
    ...
    --Wait for sit down anim to end (adaptive, capped at 6000ms client-side only)
    local sitStartTime = GetGameTimer()
    while GetGameTimer() - sitStartTime < 6000 do
        if GetEntityAnimCurrentTime(PlayerPedId(), "anim_casino_b@amb@casino@games@shared@player@", sitEnterClip) >= 0.98 then
            break
        end
        Wait(0)
    end
