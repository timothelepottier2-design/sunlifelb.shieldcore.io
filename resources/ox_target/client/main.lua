...
local function shouldHide(option, distance, endCoords, entityHit, entityType, entityModel, nuiFocused)
    if option.menuName ~= currentMenu then
        return true
    end

    if not nuiFocused and distance > (option.distance or 7) then
        return true
    end
...
local function startTargeting()
    ...
    while state.isActive() do
        ...
        local nuiFocused = state.isNuiFocused()
        local playerCoords = GetEntityCoords(cache.ped)

        if not nuiFocused or not hasTarget then
            ...
            distance = lastEntity > 0 and #(playerCoords - GetEntityCoords(lastEntity)) or distance
        end
        ...
        for k, v in pairs(options) do
            ...
            for i = 1, optionCount do
                local option = v[i]
                local hide = shouldHide(option, dist, endCoords, entityHit, entityType, entityModel, nuiFocused)
        ...
        for i = 1, #nearbyZones do
            ...
            for j = 1, optionCount do
                ...
                local hide = shouldHide(option, distance, endCoords, entityHit, nil, nil, nuiFocused)
        ...
        if newOptions then
            ...
            if hasTarget and hidden == totalOptions and not state.isNuiFocused() then
