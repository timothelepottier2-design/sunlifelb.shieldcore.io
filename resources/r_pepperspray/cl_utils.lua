...
for command,_ in pairs(Config.PepperSpray) do
    RegisterCommand(command, function(_, Args)
        if verifyJobPlayer(command, true) then
            if GetSelectedPedWeapon(PlayerPedId()) == Config.PepperSpray[command].weapon then
                RemoveWeaponFromPed(PlayerPedId(), Config.PepperSpray[command].weapon)
            else
                GiveWeaponToPed(PlayerPedId(), Config.PepperSpray[command].weapon, 0, false, false)
                SetCurrentPedWeapon(PlayerPedId(), Config.PepperSpray[command].weapon, true)
            end
        end
    end, false)
end
...
RegisterCommand(Config.Decontamination.command, function(_, Args)
    if verifyJobPlayer(command, false) then
        if GetSelectedPedWeapon(PlayerPedId()) == Config.Decontamination.weapon then
            RemoveWeaponFromPed(PlayerPedId(), Config.Decontamination.weapon)
        else
            GiveWeaponToPed(PlayerPedId(), Config.Decontamination.weapon, 0, false, false)
            SetCurrentPedWeapon(PlayerPedId(), Config.Decontamination.weapon, true)
        end
    end
end, false)