...
function Utils.LoadAnimationDict(dict)
	while not HasAnimDictLoaded(dict) do
		RequestAnimDict(dict)
		Wait(1)
	end
end
function Utils.PlayAnimation(dict, anim, flag)
	Utils.LoadAnimationDict(dict)
	TaskPlayAnim(PlayerPedId(), dict, anim, 8.0, 8.0, -1, flag, 1.0, false, false, false)
end
function Utils.PlayAnimationOnPed(ped, dict, anim, flag)
	Utils.LoadAnimationDict(dict)

	TaskPlayAnim(ped, dict, anim, 8.0, 8.0, -1, flag, 1.0, false, false, false)
end

Citizen.CreateThread(function()
	while true do
		Utils.TimeFrame = (GetGameTimer() - timer)
		timer = GetGameTimer()
		Wait(0)
	end
end)
...
function UI.LoadFont(font)
    RegisterFontFile(font[1])
    local fontId = RegisterFontId(font[2])
...
function UI.DrawSlider(screenX, screenY, width, height, backgroundColor, progressColor, value, max, settings, cb)
    ...
    elseif settings.direction == 4 then
        progressWidth = width
        ...
        if settings.NoHover ~= nil and settings.NoHover == true then
            ...
            if UI.isMouseOnButton({x = GetControlNormal(0, 239) , y = GetControlNormal(0, 240)}, {x = screenX, y = screenY}, width, height) then
                ...
                if settings.CustomHoverTexture ~= nil and settings.CustomHoverTexture ~= false then
                    ...
                    DrawSprite(settings.CustomHoverTexture[1], settings.CustomHoverTexture[2], pos[1], pos[2], width, height, heading, red, green, blue, settings.CustomHoverTexture[5])
                else
                    DrawSprite(textureDict, textureName, pos[1], pos[2], width, height, heading, red, green, blue, alpha)
                end
            else
...
function UI.Draw3DText(x,y,z,textInput,fontId,scaleX,scaleY, alpha)
    ...
    local dist = INTERACTS.GetDistanceBetweenCoords(px,py,pz, x,y,z, 1)
    ...
    SetTextColour(250, 250, 250, alpha or 255)
