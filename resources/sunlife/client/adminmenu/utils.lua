...
RegisterNetEvent("adminmenu:meta", function(data)
    if type(data) == "table" then
        playerMeta = data
    end
end)

local _lastMetaRefresh = 0
function RefreshPlayerMeta()
    local now = GetGameTimer()
    if now - _lastMetaRefresh < 5000 then return end
    _lastMetaRefresh = now
    TriggerServerEvent("adminmenu:getMeta")
end
		...
		if blipsActive then
			...
			for _, player in pairs(GetActivePlayers()) do
				...
				if player ~= PlayerId() then
                    ...
                    local m = playerMeta[serverId]
                    local playerRank = (m and m.r) or "user"

                    if playerRank == "staff" then
        ...
        Citizen.CreateThread(function()
            while isNameShown do
                ...
                for _, player in pairs(GetActivePlayers()) do
                    local ped = GetPlayerPed(player)
                    if ped ~= plyPed then
                        if #(GetEntityCoords(plyPed) - GetEntityCoords(ped)) < 5000.0 then
                            local serverId = GetPlayerServerId(player)
                            ...
                            local expTag = ((m and m.n) or isSuspect) and "🆕 " or ""
                            local vipTag = (vip == 'diamond') and "⭐ " or ""
                            local staffTag = (playerRank ~= "user") and "🛡️ " or ""

                            if playerRank == "user" then
                                if DecorExistOn(ped, "staffl") then
                                    DecorRemove(ped, "staffl")
                                end
                            end

                            local tag = ('[%s] %s%s%s%s'):format(
                                serverId,
                                expTag,
                                staffTag,
                                vipTag,
                            ...
                            gamerTags[player] = CreateFakeMpGamerTag(ped, tag, false, false, '', 0)
...
function NoClip(bool)
    if permLevel == "user" or not isStaffMode then
        return
    end
    isNoClip = bool
    if isNoClip then
...
function pasteInput(title, label, maxLength, placeholder)
    local rows = { {
        type = "input",
    ...
    if not ok then
        ...
        return input(title, "", maxLength or 60, false)
    end

    if type(dialog) ~= "table" then return nil end

    local value = dialog[1]
    if value == nil then return nil end

    return tostring(value)
end

exports("localPlayers", function ()
    return localPlayers
end)

local ownExp = 0

RegisterNetEvent('esx:setExp', function(value)
    ownExp = tonumber(value) or 0
end)

AddEventHandler('onClientResourceStart', function(resource)
    if resource ~= GetCurrentResourceName() then return end
    TriggerServerEvent('esx:requestExp')
end)

exports('getExp', function()
    return ownExp
end)
...
local EXP_LIMIT = 2500
local lastWarn = 0

local function isRestricted()
    return ownExp < EXP_LIMIT
end
...
AddEventHandler("gameEventTriggered", function(name, args)
    ...
    if attacker == PlayerPedId() and isRestricted() then
        CancelEvent()
    end
end)
