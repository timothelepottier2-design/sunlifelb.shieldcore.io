...
RegisterCommand("o",function()
	if onCooldown then
		drawNativeNotification("~r~Vous devez attendre avant de prendre un otage !")
		return
	end
	takeHostage()
	cooldown()
...
function takeHostage()
	if exports["sunlife"]:InZoneSafe() then
		drawNativeNotification("~r~Vous ne pouvez pas prendre un otage ici !")
		return
	end
	...
	if currentWeapon and currentWeapon ~= 0 and currentWeapon ~= UNARMED_HASH then
		canTakeHostage = true
		foundWeapon = currentWeapon
	end

	if not canTakeHostage then
		drawNativeNotification("Vous devez sortir une arme (pas les poings) !")
	end
	...
	if not holdingHostageInProgress and canTakeHostage then
		...
		local closestPlayer = F5_GetClosestPlayer(2)
		local target = GetPlayerServerId(closestPlayer)
		if IsEntityDead(GetPlayerPed(GetPlayerFromServerId(target))) then
			return
		end
