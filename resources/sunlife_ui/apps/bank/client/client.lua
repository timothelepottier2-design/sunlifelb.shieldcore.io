...
local function SendNUIMessage(data)
    return __SL_send_native({ _slapp = 'bank', _slpayload = data })
end
local function SetNuiFocus(hasFocus, hasCursor)
    __SL_send_native({ _slrouter = true, _slapp = 'bank', focus = hasFocus and true or false })
    return __SL_focus_native(hasFocus, hasCursor)
end
local function RegisterNUICallback(name, cb)
    return __SL_reg_native('bank/' .. name, cb)
end
...
local banks = {
	...
	{name="Banque", id=108, x=7168.796875, y=370.163391, z=58.041916}
}
...
local atms = {
	...
	{name="ATM", id=277, x=-32.31, y=-1104.23, z=27.27},
	{name="ATM", id=277, x=816.0067, y=-2147.049, z=29.39713},
	{name="ATM", id=277, x=797.6279, y=-766.7502, z=26.78081},
	{name="ATM", id=277, x=-1729.482, y=-780.6854, z=10.24445},
	{name="ATM", id=277, x=-927.1061, y=-2036.9492, z=8.5012},
	{name="ATM", id=277, x=-1315.520508, y=-835.589111, z=16.961533},
}
	...
	Citizen.CreateThread(function()
		while true do
			...
			if nearBank() or nearATM() then
				nearThing = true
				DisplayHelpText("Appuyez sur ~INPUT_PICKUP~ pour accèder à votre compte")
...
RegisterNUICallback('deposit', function(data, cb)
	TriggerServerEvent('bank:deptozosit', tonumber(data.amount))
	TriggerServerEvent('bank:balance')
	if cb then cb({ ok = true }) end
end)

RegisterNUICallback('withdrawl', function(data, cb)
	TriggerServerEvent('bank:withdtozraw', tonumber(data.amountw))
	TriggerServerEvent('bank:balance')
	if cb then cb({ ok = true }) end
end)
...
RegisterNUICallback('getJournal', function(_, cb)
	TriggerServerEvent('bank:getJournal')
	if cb then cb({}) end
end)

RegisterNetEvent('bank:journal:data')
AddEventHandler('bank:journal:data', function(rows)
	SendNUIMessage({ type = 'journal', rows = rows or {} })
end)
...
local BANK_BLIP_CATEGORY = 11

local function _createBankBlip(entry, sprite, color, gxtKey, label)
	local b = AddBlipForCoord(entry.x, entry.y, entry.z)
	SetBlipSprite(b, sprite)
	SetBlipDisplay(b, 4)
	SetBlipScale(b, 0.5)
	SetBlipColour(b, color)
	SetBlipAsShortRange(b, true)
	SetBlipCategory(b, BANK_BLIP_CATEGORY)
	BeginTextCommandSetBlipName(gxtKey)
	EndTextCommandSetBlipName(b)
	return b
end

CreateThread(function()
	AddTextEntry("BN_SNL_BANK", "Banque")
	AddTextEntry("BN_SNL_ATM",  "Distributeur (ATM)")

	for i = 1, #banks do
		_createBankBlip(banks[i], 108, 2, "BN_SNL_BANK", "Banque")
	end
	for i = 1, #atms do
		_createBankBlip(atms[i], 277, 2, "BN_SNL_ATM", "Distributeur (ATM)")
	end
end)
