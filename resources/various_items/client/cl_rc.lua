...
RCCar.Start = function()
	...
	while DoesEntityExist(RCCar.Entity) and DoesEntityExist(RCCar.Driver) do
		...
		if distanceCheck <= ConfigRC.LoseConnectionDistance then
...
RCCar.Spawn = function()
	...
	print(('^5[NETDIAG][VEHICLE]^7 %s cl_rc.lua:171 CreateVehicle NETWORKED rcbandito'):format(GetCurrentResourceName()))
	...
	print(('^6[NETDIAG][PED]^7 %s cl_rc.lua:177 CreatePed NETWORKED driver'):format(GetCurrentResourceName()))
...
RCCar.ToggleCamera = function(boolean)
	if not ConfigRC.Camera then return end
