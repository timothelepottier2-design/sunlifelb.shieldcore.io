...
--
-- Volontairement réduit aux 3 sirènes actives (Wail, Yelp, Priority) + l'Airhorn
-- en index 1 (utilisé par le code comme "ARHRN" séparé, RegisterKeyMaps skip i=1).
-- → on enregistre donc 3 RegisterKeyMapping côté client (1ère, 2ème, 3ème sirène).
-- Si besoin de réactiver une tonalité, dé-commenter la ligne correspondante ET
-- l'ajouter dans SIREN_ASSIGNMENTS.DEFAULT plus bas (sinon pas de tonalité jouée).
SIRENS = {
...
--
-- DEFAULT couvre toutes les vehicles emergency. Les profils spécifiques
-- (FIRETRUK / AMBULAN / LGUARD) référençaient les indices 11-14 (Powercall et
-- Fire Horn/Yelp/Wail) qui ne sont plus définis dans SIRENS ci-dessus → on les
-- commente. Tout véhicule emergency tombera sur DEFAULT (Wail/Yelp/Priority).
SIREN_ASSIGNMENTS = {
	--['<gameName>'] = {tones},
	-- Position 1 = Airhorn (réservé bouton ARHRN, pas de keybind sirène),
	-- Positions 2..4 = Wail / Yelp / Priority → keybinds 1ère / 2ème / 3ème sirène.
	['DEFAULT'] = { 1, 2, 3, 4 },
